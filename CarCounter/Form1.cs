    ...
    public partial class frmMain : Form
    {
        ...
        private void process(object sender, EventArgs e)
        {
            ...
            using (Mat imgFrame = video.QueryFrame())
            {
                ...
                {
                    foreach(Rectangle detectedRect in rects)
                    {
                        smartAdd(detectedRect);
                    }
                    foreach (Rectangle rect in cars)
                    {
                        output.Draw(rect, new Bgr(Color.Red), 2);
                    }
        ...
        private void smartAdd(Rectangle newRect)
        {
            bool available = false;
            for(int i = 0; i < cars.Count; i++)
            {
                if (cars[i].Contains(newRect))
                {
                    available = true;
                    break;
                }
